...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Load data
        fetchReports();
        checkUserAdminStatus(currentUser.getUid());
    }

    private void checkUserAdminStatus(String userId) {
        FirebaseUser currentUser = mAuth.getCurrentUser(); // Tambahkan baris ini
        ...
        db.collection("users").document(userId)
                ...
                .addOnSuccessListener(documentSnapshot -> {
                    if (documentSnapshot.exists()) {
                        ...
                        // Lanjutkan inisialisasi MainActivity jika bukan admin
                        initializeViews();
                        setupListeners();
                        fetchUserData(currentUser);
                        fetchReports();
                ...
                .addOnFailureListener(e -> {
                    ...
                    Toast.makeText(this, "Gagal memeriksa status admin: " + e.getMessage(), Toast.LENGTH_SHORT).show();
                    mAuth.signOut();
                    startActivity(new Intent(this, LoginActivity.class));
                    finish();
                });
        ...
        if (resultCode == RESULT_OK && data != null) {
            switch (requestCode) {
                case ADD_REPORT_REQUEST:
                    ...
                    if (newReport != null) {
                        // Cek apakah laporan sudah ada di list
                        boolean exists = reportsList.stream()
                                .anyMatch(report -> report.getId().equals(newReport.getId()));
                ...
                .addSnapshotListener((value, error) -> {
                    ...
                    if (value != null && !value.isEmpty()) {
                        for (QueryDocumentSnapshot document : value) {
                            ...
                            // Pastikan ID dokumen diset
                            report.setId(document.getId());

                            // Tambahkan log untuk debugging
                            Log.d(TAG, "Fetched Report: " + report.getId() + " - " + report.getTitle());

                            // Tambahkan laporan ke list
                    ...
                    // Tambahkan log untuk jumlah laporan
                    Log.d(TAG, "Total Reports: " + reportsList.size());
